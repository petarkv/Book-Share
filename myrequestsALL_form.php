...
<?php
    ...
    $booksall = DB::getInstance()->query("SELECT borrow.id_request,borrow.is_requestes, users.username, users.name,
...
<tr>
    ...
    <td id="rowbook"><?php if ($book->is_requestes == 0) {
        echo '<p style="color: green;">PENDING</p>';
    }elseif(($book->is_requestes == 1))
    {
...
if($current_page>1)
{
    echo '<a href="myrequestsALL_form.php?page='.($current_page-1).'">&laquo Previous</a>';
}
...
foreach ($numbers as $num)
{
    ...
    if($num == $current_page){
        ...
        echo '<a href="myrequestsALL_form.php?page='.$num.'">'.$num.'</a>';
}


if($current_page<$num)
{
    echo '<a href="myrequestsALL_form.php?page='.($current_page+1).'">Next &raquo</a>';
}
echo '</div>';
?>
